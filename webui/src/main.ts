...
document.querySelector<HTMLDivElement>('#app')!.innerHTML = /* html */ `
  <section class="header">
    <div id="title" class="search-hide">${config.identity === 'OMK' ? i18n.t('omk_header_title') : i18n.t('header_title')}</div>
...
// OMK-only features
mainMenu.on('menu-omk-device', () => dialogController.showOmkDevice())
mainMenu.on('menu-omk-raw-config', () => dialogController.showOmkRawConfig())
mainMenu.on('menu-omk-logs', () => dialogController.showOmkLogs())
mainMenu.on('menu-omk-restart-keymint', () => restartDaemon('keymint'))
mainMenu.on('menu-omk-restart-injector', () => restartDaemon('injector'))
mainMenu.on('menu-omk-restart-all', () => restartDaemon('all'))
...
// OMK submenu + uninstall button visibility
const isOmk = config.identity === 'OMK'
if (!isOmk) {
...
async function restartDaemon(which: 'keymint' | 'injector' | 'all'): Promise<void> {
  try {
    await cli.restartDaemon(which)
    snackbar.show(i18n.t('omk_restart_requested', which))
  } catch {
    snackbar.show(i18n.t('omk_restart_error'), false)
  }
}
