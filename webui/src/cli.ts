...
export class Cli {
  ...
  // Returns the first existing module.prop path. Addon modules keep a hidden
  // copy at common/update/module.prop; built-in modules (e.g. Oh My Keymint)
  // keep it at the module root.
  async getModulePropPath(): Promise<string> {
    const basePath = await this.getBasePath()
    for (const candidate of [`${basePath}/common/update/module.prop`, `${basePath}/module.prop`]) {
      if (await File.exist(candidate)) return candidate
    }
    return `${basePath}/module.prop`
  }
...
while IFS= read -r line || [ -n "$line" ]; do
  ...
  // OMK-only: signal the watchdog to restart a daemon by touching the
  // corresponding sentinel file under /data/adb/omk.
  async restartDaemon(which: 'keymint' | 'injector' | 'all'): Promise<void> {
    const sentinel = `/data/adb/omk/restart.${which}`
    const result = await exec(`mkdir -p /data/adb/omk && touch '${sentinel}'`)
    if (result.errno !== 0) throw new Error(`restartDaemon(${which}) failed (${result.errno})`)
  }
