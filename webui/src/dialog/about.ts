...
export class AboutDialog {
  ...
  getElement(): DocumentFragment {
    const isOmk = this.#config.identity === 'OMK'
    const nameLine1 = isOmk ? i18n.t('omk_about_name_line1') : i18n.t('about_module_name_line1')
    const nameLine2 = isOmk ? i18n.t('omk_about_name_line2') : i18n.t('about_module_name_line2')
    const author = isOmk ? 'James Clef' : 'KOWX712'
    const disclaimer = isOmk ? i18n.t('omk_about_disclaimer') : i18n.t('about_disclaimer')
    const template = document.createElement('template')
    template.innerHTML = /* html */ `
      <md-dialog id="about-dialog">
        <div slot="headline" class="about-headline">
          <div class="about-title-row">
            <div id="module_name_line1">${nameLine1}</div>
            <div id="working-mode-tag">${this.#config.identity}</div>
          </div>
          <div id="module_name_line2">${nameLine2}</div>
          <div id="module-version"></div>
          <div id="author"><span id="authored">${i18n.t('about_by')}</span> ${author}</div>
        </div>
        <div slot="content">
          ...
          <div class="link">
            ...
            <md-filled-button id="canary"${isOmk ? ' style="display:none"' : ''}>
              <span>${i18n.t('about_canary_update')}</span>
              <md-icon slot="icon">science</md-icon>
            </md-filled-button>
            <md-filled-button id="locales"${isOmk ? ' style="display:none"' : ''}>
  ...
  async #loadModuleVersion(): Promise<void> {
    try {
      const propPath = await this.#cli.getModulePropPath()
      let version = await this.#cli.grepProp('version', propPath)
      if (import.meta.env.DEV) version = 'v1.0 (690581)'
      if (version) {
