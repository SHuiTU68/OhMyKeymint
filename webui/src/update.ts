...
const UPDATE_JSON_URL = `${RAW_URL}/changelog/update.json`
const NIGHTLY_LINK = `https://nightly.link/${GITHUB_REPO}/workflows/ci/main?preview`
...
export class UpdateManager {
  ...
  async #getLocalVersionCode(): Promise<number> {
    try {
      ...
      const value = await this.#cli.grepProp('versionCode', propPath)
  ...
  async showModule(show: boolean): Promise<void> {
    try {
      ...
      // Only the addon (TA_utl) hides its module.prop behind common/update/.
      // Built-in modules like Oh My Keymint keep a real module.prop at the
      // root and must never be deleted/restored here.
      if (!(await File.exist(basePath + UPDATE_PROP))) return
      if (show) {
