...
DEFAULT_BSSL_SYS_PATH = REPO_ROOT / "boringssl" / "rust" / "bssl-sys"
...
def render_config(
    ...
    lines = [
        ...
        f"ANDROID_NDK = {toml_string(str(ndk_root))}",
        f"ANDROID_NDK_HOME = {toml_string(str(ndk_root))}",
        f"ANDROID_NDK_ROOT = {toml_string(str(ndk_root))}",
        f"NDK_HOME = {toml_string(str(ndk_root))}",
    ...
    if bssl_sys_path is not None:
        lines.extend(
            [
                "[patch.crates-io]",
                f"bssl-sys = {{ path = {toml_string(str(bssl_sys_path))} }}",
                "",
            ]
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Optional path to a generated boringssl/rust/bssl-sys directory. "
            "Defaults to boringssl/rust/bssl-sys inside the repository when present."
        ),
    )
...
def main() -> int:
    ...
    try:
        ...
        bssl_sys_path = args.bssl_sys_path
        if bssl_sys_path is None and DEFAULT_BSSL_SYS_PATH.is_dir():
            bssl_sys_path = DEFAULT_BSSL_SYS_PATH
        if bssl_sys_path is not None:
            bssl_sys_path = bssl_sys_path.expanduser().resolve()
            if not bssl_sys_path.is_dir():
                raise FileNotFoundError(f"Generated bssl-sys directory not found: {bssl_sys_path}")
        rendered = render_config(ndk_root, host_tag, args.api_level, bssl_sys_path)
